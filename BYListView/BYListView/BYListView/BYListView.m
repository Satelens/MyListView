...
@interface BYListViewCell : UITableViewCell
@property(nonatomic, strong)NSIndexPath *indexPath;
@end
@implementation BYListViewCell
...
//        POST_NOTIFY(XTITNotifyFavoriteCellLongPressChanged, nil, @{Notify_UserInfo_Key_IndexPath: self.indexPath})
        // 长安手势事件
    }
}
...
@interface BYListView () <UITableViewDelegate, UITableViewDataSource>
...
-(void)initialize
{
    ...
    self.selectionStyle = BYListViewSelectionStyleGray;
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if ([self.delegate respondsToSelector:@selector(listView:reuseViewForHeader:inSection:inColumn:)]) {
        ...
        for (int column=0; column<numberOfColumn; column++) {
            ...
            if (column == 0) {
                view.frame = CGRectMake(_curContentOffset, 0, [self columnWidthInColumn:column], [self.delegate listView:self heightForHeaderInSection:section]);
            }
            x += [self columnWidthInColumn:column];
            if (!view.superview) {
                [headerView.contentView addSubview:view];
                if (column == 0) {
                    [headerView.contentView bringSubviewToFront:view];
                }
            }
        }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BYListViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (!cell) {
        cell = [[BYListViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
        ...
        if (self.selectionStyle == BYListViewSelectionStyleNone) {
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    if([keyPath isEqualToString:@"contentOffset"])
    {
        ...
        for (BYListViewCell *cell in visibleCells) {
        ...
        _curContentOffset = self.scrollView.contentOffset.x;
